...
export function NewsletterForm({ className }: NewsletterFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      toast.success(res.message || 'Thanks! We will be in touch soon.', { className: 'toast-premium' });
      setEmail('');
    } catch (error) {
  ...
  return (
    <form
      ...
      className={cn(
        'flex items-stretch border-b border-[var(--color-line)] focus-within:border-[var(--color-gold)] transition-colors duration-300',
        className
      )}
    >
      <input
